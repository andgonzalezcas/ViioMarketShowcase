    ...
    <div className="flex h-screen w-full items-center justify-center bg-form-bg bg-center bg-no-repeat bg-cover text-xs" >
      <div className="max-w-[80vw] md:max-w-md w-full h-[80vh] pt-16 px-8 pb-6 rounded-xl backdrop-blur-md bg-background bg-opacity-50 flex flex-col justify-between text-center">
        <div className="flex flex-col items-center text-center">
          ...
          <h1 className="max-w-[240] mt-8 text-2xl md:text-4xl opacity-70 font-bold">{"REGISTRARSE"}</h1>
        </div>
        <form onSubmit={(e) => handleLogin(e)} className="w-full flex flex-col items-center md:text-base">
          <input className="w-full max-w-[320px] bg-background text-center p-2 rounded-lg" placeholder="usuario" />
          <p className="w-full mb-1 pb-1 h-5 md:h-7"></p>
        ...
        <Link to={'/login'} className="w'full flex justify-center">
          <p className="w-fit text-xs md:text-base hover:underline hover:underline-offset-1 cursor-pointer transition-all duration-300">{"Inicar Sesión"}</p>
        </Link>
      </div>
    </div>
  )
